...
public class CameraSource {
    ...
    private Camera createCamera() throws IOException {
        ...
        if (requestedAutoFocus) {
            if (parameters
                ...
                Debug.logDebug("Camera auto focus is not supported on this device.");
            }
        }
    ...
    private static List<SizePair> generateValidPreviewSizeList(Camera camera) {
        ...
        if (validPreviewSizes.size() == 0) {
            Debug.logDebug("No preview sizes have a corresponding same-aspect-ratio picture size");
            for (Camera.Size previewSize : supportedPreviewSizes) {
    ...
    private void setRotation(Camera camera, Camera.Parameters parameters, int cameraId) {
        ...
        switch (rotation) {
            ...
            case Surface.ROTATION_270:
                ...
                break;
            default:
                Debug.logDebug("Bad rotation value: " + rotation);
        }
        ...
        void setNextFrame(byte[] data, Camera camera) {
            synchronized (lock) {
                ...
                if (!bytesToByteBuffer.containsKey(data)) {
                    Debug.logDebug("Skipping frame. Could not find ByteBuffer associated with the image data from the camera.");
                    return;
                }
